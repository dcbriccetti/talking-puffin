...
class UserPropertiesDialog(userPrefs: Preferences, status: Status) extends Frame with Cancelable {
  private val screenName = status.getUser.getScreenName
  ...
  contents = new GridBagPanel {
    ...
    val nameField = new TextField(props.getName(status.getUser.getName)) {columns=40}
    ...
    add(new FlowPanel(FlowPanel.Alignment.Left)() {
      val saveAction = new Action("Save") {
        ...
        def apply = {
          if (nameField.text == status.getUser.getName) props.removeName else props.putName(nameField.text)
...
object UserProperties {
  ...
  def overriddenUserName(userPrefs: Preferences, user: User) =
    if (UserProperties.exists(userPrefs, user.getScreenName))
      new UserProperties(userPrefs, user.getScreenName).getName(user.getName)
    else user.getName
